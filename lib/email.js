...
export async function sendTeamInvite(toEmail, teamName, inviterName, inviteLink) {
  if (!process.env.RESEND_API_KEY) {
    console.warn('RESEND_API_KEY is not defined. Skipping invite email.');
    return;
  }
  ...
  try {
    await resend.emails.send({
      from: 'HackRadar <onboarding@resend.dev>',
      to: toEmail,
      subject: `🤝 ${inviterName} invited you to join "${teamName}" on HackRadar!`,
      html: `
        <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 0; color: #0f172a; background-color: #faf5ff;">
          <div style="background: linear-gradient(135deg, #7c3aed 0%, #c026d3 100%); padding: 40px 30px; text-align: center; border-radius: 0 0 24px 24px;">
            <h1 style="color: white; font-size: 28px; margin: 0 0 8px 0; letter-spacing: -0.5px;">You're Invited! 🎉</h1>
            <p style="color: rgba(255,255,255,0.85); font-size: 15px; margin: 0;">Join a hackathon team on HackRadar</p>
          </div>
          
          <div style="padding: 32px 30px;">
            <p style="font-size: 16px; line-height: 1.6; margin-bottom: 24px;">
              Hey there! <strong>${inviterName}</strong> has invited you to join their team:
            </p>
            
            <div style="background: white; border: 2px solid #e9d5ff; border-radius: 16px; padding: 24px; margin-bottom: 28px; text-align: center;">
              <h2 style="color: #7c3aed; font-size: 22px; margin: 0 0 8px 0;">${teamName}</h2>
              <p style="color: #64748b; font-size: 13px; margin: 0;">Click the button below to view team details and join</p>
            </div>
            
            <div style="text-align: center; margin-bottom: 32px;">
              <a href="${inviteLink}" style="background: linear-gradient(135deg, #7c3aed, #c026d3); color: white; padding: 14px 36px; text-decoration: none; border-radius: 12px; font-size: 16px; font-weight: bold; display: inline-block; letter-spacing: 0.3px;">
                View Invite & Join Team
              </a>
            </div>
            
            <p style="font-size: 13px; color: #94a3b8; line-height: 1.5;">
              Or copy this link: <a href="${inviteLink}" style="color: #7c3aed; word-break: break-all;">${inviteLink}</a>
            </p>
          </div>
